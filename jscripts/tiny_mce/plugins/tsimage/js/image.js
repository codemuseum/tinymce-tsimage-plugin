var ImageDialog = {
  TS : {
    ...
    initUploadFrame : function() {
      if (!this.uploadFrameCreated) {
        document.getElementById('authenticity_token').value = tinyMCEPopup.getParam("authenticity_token");
      	var iframe1=ImageDialog.TS.createTagHelper('iframe','tinymce_image_upload_frame'); 
    ...
    onImageUploading : function() {
      document.getElementById('loading_alert').style.display="inline";
    },
    createTagHelper : function(tag,name){
    ...
    setImage : function(url, title_text, alt_text){
    	var f_src = document.forms[0], f = document.forms[0];
    	f_src.src.value = url;
    	f.title.value = title_text;
    	f.alt.value = alt_text;
    	ImageDialog.showPreviewImage(f_src.src.value, 1);
    ...
    fetchGalleryImages : function() {
        ...
        // params : p,
				...
				error : function(e, x) {
					tinyMCEPopup.editor.windowManager.alert(e.errstr || ('Error response: ' + x.responseText));
				}
			});
    },
    showGalleryImages : function(elId, html) {
      tinyMCEPopup.dom.setHTML(elId, html);
    }
  },
...
// Since we're not in the same window as the main page, we can overwrite builtin javascript methods at will.  Sort of a hack
var TS = {
  Assets : { 
    Selector : { 
      Pictures : { 
        uploaded: function(data) {
          ImageDialog.TS.setUploadedImage(data);
        }
      }
    }
  }
